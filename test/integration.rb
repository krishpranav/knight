...
class KnightTest < Minitest::Test
  ...
  def test_version
    p = IO.popen(['./knight', '--version'], 'r+')
    res = p.read.to_s
    p.close
    assert res
    assert res =~ %r{^knight version ([\d\.]+(-dev)?) \( https://morningstarsecurity.com/research/knight/ \)$}
  end

  def test_usage
    p = IO.popen(['./knight', '--help'], 'r+')
    res = p.read.to_s
    p.close
    assert res
    assert res =~ /^knight - web security scanner version ([\d\.]+(-dev)?)\.$/
  end

  def test_invalid_url
    res = IO.popen(['./knight','--color', 'never', 'https://'], :err => [ :child, :out ]).read
  ...
  def test_verbose_output
    ...
    p.close
    assert res
    assert res.split("\n").flatten.first =~ %r{knight report for https://#{@test_host}/$}
  end
  ...
  def test_plugins
    ...
    p.close
    assert res
    assert res =~ %r{^https://#{@test_host}/ \[200 OK\] HTTPServer\[Apache\], Title\[knight - web security scanner.\]$}
  end

  def test_custom_plugin
    p = IO.popen(['./knight',
      '--color', 'never',
      '--custom-plugin', ':text=>"knight -  web security scanner."',
      "https://#{@test_host}/"], 'r+')
    res = p.read.to_s
    p.close
    assert res
  ...
  def test_url_prefix
    ...
    p.close
    assert res
    assert_match %r{knight - web seceurity scanner.}, res
  end
