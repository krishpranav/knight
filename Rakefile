require 'bundler'
require 'fileutils'
require File.expand_path( File.dirname( __FILE__ ) ) + '/lib/knight'
...
begin
    ...
    namespace :spec do
        ...
        RSpec::Core::RakeTask.new( :core ) do |t|
            t.pattern = FileList[ 'spec/knight/**/*_spec.rb' ]
        end
        ...
        namespace :generate do
            task :afr do
                begin
                    ...
                    Knight::Data.issues.store
                    $spec_issues.each { |i| Knight::Data.issues << i }

                    Knight::Options.url = 'http://test.com'
                    Knight::Options.audit.elements Knight::Page::ELEMENTS - [:link_templates]
                    Knight::Options.audit.link_templates = [
                        /\/input\/(?<input>.+)\//,
                        /input\|(?<input>.+)/
                    ]

                    Knight::Report.new(
                        sitemap: { Knight::Options.url => 200 },
                        issues:  Knight::Data.issues.sort
                    ).save( 'spec/support/fixtures/report.afr' )
                ensure
                    Knight::Options.reset
                    Knight::Data.reset
                end
            end
        end
...
desc 'Build knight and run all the tests.'
